...
def init_wait_time(client):
    ...
    for row in conn.query(query_get_all_symbol):
        symbol = row[0]
...
def buy_logic(client, symbol):
    ...
    try:
        conn = SQLite(DB_CONFIG)
        order_market_buy(client, symbol, alpha_qty=15)
...
def init_symbol_data(client):
    ...
    scheduler.add_job(init_wait_time, 'cron', hour=11, id='init_wait_time', args=(client,))
    scheduler.start()
